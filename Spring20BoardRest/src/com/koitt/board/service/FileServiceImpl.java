...
public class FileServiceImpl<T> implements FileService<T>{
	...
	public String add(HttpServletRequest request, MultipartFile attachment) throws FileException {
		try {
			...
			if(attachment != null && !attachment.isEmpty()) {
				...
				// 나중에 파일을 다운받을 때 글자깨짐을 방지하기 위해 URL 인코딩(UTF-8값은 파일명의 실제 인코딩 방식을 뜻한다)
	...
	public void download(HttpServletRequest request, HttpServletResponse response, String filename)
		...
		try {
			 ...
			 *  MIME TYP : EsetContentType("application/octet-stream") 
			 *  " " 안에 들어갈 타입을 전송해서 브라우저가 준비하게끔 하기 위해 작성
			 *  "application/octet-stream" : 모든 유형(text, jpg..) 받을 수 있음  
			 *  
			 */ 
			 ...
			 * 설정 안할 경우 브라우저는 기존에 있다고 인식 하여 받지않고 기존 파일을 사용
			 */
