...
public class Example {
	...
	/*
	 * getMessage 메소드
	 * 첫 번재 파라미터 : properties 파일의 code(변수명)
	 * 두 번째 파라미터 : {0}와 같이 빈칸에 들어갈 문자열 배열
	 * 			예) new Object[]{"Apple"}을 전달하면 {0} 대신 Apple로 대치
	 * 			   new Object[]{"Car", "Computer"}를 전달하면 {0} 대신에 Car
	 * 				{1} 대신에 Computer로 대치
	 * 세 번재 파라미터: 첫 번째 파라미터에 해당하는 code(변수명)이 없을 경우 기본으로
	 * 				출력하는 문자열
	 ...
	 */
	public void execute() {
		String message01 = this.messages.getMessage("greeting", null, "Hi", Locale.KOREAN);
